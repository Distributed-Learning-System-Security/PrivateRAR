...
class EncodedNumber(object):
    ...
    Notes:
      ...
      表示有符号数相对容易，使用 Paillier 方案的模算数特性，选择介于以下范围内的整数
      位于max_int和n-max_int之间的值范围被保留用于检测溢出
      ...
      在固定精度模式下，您可以通过将每个浮点数乘以一个大数（例如1e6）并将得到的乘积四舍五入来进行编码。
      您可以通过将结果除以该数字来进行解码。但是，此编码方案不满足上述属性＃2：在每次乘法之后，您必须除以这个大数。
      在Paillier方案中，除非解密，否则无法做到这一点。对于某些任务，这是可以接受或可以解决的，但对于其他任务则无法解决。
        ...
        *max_exponent*, and such that the number *precision* is not
        ...
        if precision is None:
            if isinstance(scalar, int):
                ...
                # 将浮点数的指数部分bin_flt_exponent减去浮点数尾数的位数cls.FLOAT_MANTISSA_BITS，得到二进制下最小可表示的数的指数部分
                # 用来得到浮点数中最小可表示的数值，也称为最小二进制位数（LSB）。
