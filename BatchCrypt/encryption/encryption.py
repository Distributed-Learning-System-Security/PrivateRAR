...
def stochastic_r(ori, frac, rand):
    ...
    for i in prange(len(ori)):
        # 对于每个浮点数，根据其小数部分和随机数数组的值，决定是向下舍入还是向上舍入，并将结果存储在一个数组中。
        if frac[i] >= 0:
...
# 一个随机舍入函数（stochastic_round），用于将一个NumPy数组中的浮点数随机舍入为整数
def stochastic_round(ori):
    # 函数生成一个与ori数组相同大小的随机数数组rand，用于决定每个浮点数是应该向下舍入还是向上舍入。
    rand = np.random.rand(len(ori))
    # np.modf函数将每个浮点数分解为整数部分和小数部分
...
# 对矩阵进行批处理加密，相当于将矩阵划分为 16 个小块进行加密
def encrypt_matrix_batch(public_key: PaillierPublicKey, A, batch_size=100, bit_width=8, pad_zero=3, r_max=0.5):
    og_shape = A.shape
    if len(A.shape) == 1:
        A = np.expand_dims(A, axis=0)
    ...
    # batched_nums 是一个初始化 idx_range 长度的一维数组, 每个元素是一个 2048 位的二进制数组, 均为 0
    batched_nums = np.array([pow(2, 2048)] * idx_range)
    batched_nums *= 0
    # print(batched_nums.dtype)
    # 将矩阵按照batch_size进行划分，并将每个小块拼接成一个大整数。
    for i in range(batch_size):
        idx_filter = [i + x * batch_size for x in idx_base]
        # print(idx_filter)
        filted_num = A[idx_filter]
        # print([bin(x) for x in filted_num])
        # batched_nums 左移 bit_width + pad_zero 位，然后与 filted_num 进行按位或运算
        # 得到一个新的一维数组，其中每个元素都是一个2048位的二进制整数。只是对batch_nums 中的每个数都左移了 bit_width + pad_zero bit，然后与 filted_num 执行或运算，即拼接上去
        # batched_nums 中的每一个元素，其实对应的是一个块，其中会有 batch_size 个数的二进制数会进行拼接，然后去加密
...
def restore_shape(component, shape, batch_size=16, bit_width=8, pad_zero=3):
    ...
    for i in range(batch_size):
        # filter_ 从低到高依次去 11 bits，0b11111111111  ， 0b1111111111100000000000
