...
bit_width = 8
pad_zero = 3
filter_ = (pow(2, bit_width + pad_zero) - 1) << ((bit_width + pad_zero) * 1)

class my_text(object):
    scalar = 0.012
    FLOAT_MANTISSA_BITS = sys.float_info.mant_dig
    LOG2_BASE = 4

    @classmethod
    def encode(cls, scalar):
        x = math.frexp(scalar)[0]
        ...
        print("Scalar: ", scalar)
        print("x: ", x)
        print("flt_exponent: ", flt_exponent)
        print("lsb_exponent: ", lsb_exponent)
        print("exponent: ", exponent)
        
        # 从编码中恢复出原始值
        flt_exponent = exponent * cls.LOG2_BASE
        lsb_exponent = cls.FLOAT_MANTISSA_BITS - flt_exponent
        scalar = x * pow(2, flt_exponent) * pow(2, lsb_exponent)
        print("Recovered scalar: ", scalar)

# 创建 my_text 类的实例并测试 encode() 函数
a = my_text()
a.encode(0.012)
...
print("gaussian",np.array(alphas))

for i in range(2,9):
    print(mse_gaussian(x, sigma=sigma, num_bits=m))
...
if __name__ == '__main__':
    ...
    for num_bits in range(2, 17):
        ...
        print("Gaussian clipping value: {}".format(get_alpha_gaus(values, values.size,num_bits)))
        print("------")
